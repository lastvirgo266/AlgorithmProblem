...
int main(){
    ...
    while(1){
        ...
        if((0<= new_y && new_y <N &&
           ...
           0> new_y || new_y >=N || 0> new_x || new_x >= N){
            int new_direction[2];

            group[0].direction[0] *= -1;
            group[0].direction[1] *= -1;
            arr_chess[group[0].number].direction[0] *= -1;
            arr_chess[group[0].number].direction[1] *= -1;

            new_direction[0] = group[0].direction[0];
            new_direction[1] = group[0].direction[1];

            int blue_y = temp.y + new_direction[0];
            int blue_x = temp.x + new_direction[1];


                new_y = blue_y;
                new_x = blue_x;

            if( !(0<= blue_y && blue_y<N && 0<= blue_x && blue_x <N &&
                map[blue_y][blue_x] != BLUE) )
